package io.lunarchain.lunarcoin.config
...
class BlockChainConfig {
        ...
        if (config.hasPath("peer.privateKey")) {
            ...
            if (file.canRead()) {
                ...
                OutputStreamWriter(FileOutputStream(file), "UTF-8").use({ w ->
                    props.store(
                        w,
                        "Auto generated NodeID, you can set 'peer.privateKey' to replace the auto generated NodeID."
                    )
                })
    ...
    fun getClientId(): String {
        return "LunarCoin Kotlin Client"
    }
