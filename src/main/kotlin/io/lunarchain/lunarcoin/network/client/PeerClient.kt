package io.lunarchain.lunarcoin.network.client
...
import io.lunarchain.lunarcoin.network.message.HelloMessage
import io.lunarchain.lunarcoin.network.message.MessageCodes
import io.lunarchain.lunarcoin.network.message.handler.MessageDecodeHandler
...
class PeerClientHelloMessageHandler(val node: Node, val manager: BlockChainManager) : ByteToMessageDecoder() {
    ...
    fun sendHelloMessage(channel: Channel) {
        ...
        val msg = HelloMessage(
            config.getPeerVersion(), config.getClientId(), config.getPeerListenPort(),
            config.getNodeId()
        )
